...
Nodo* crear_nodo(unsigned long long id, const char *mensaje) {
    ...
    n->id = id;
    n->mensaje = strdup(mensaje ? mensaje : "");
...
Blockchain* crear_blockchain() {
    Blockchain *b = malloc(sizeof(Blockchain));
    b->primero = b->ultimo = NULL;
    b->longitud = 0;
    return b;
}

void liberar_blockchain(Blockchain *b) {
    if (!b) return;
...
void blockchain_agregar_con_id(Blockchain *b, unsigned long long id, const char *mensaje) {
    Nodo *n = crear_nodo(id, mensaje);
    if (!b->primero) {
        b->primero = b->ultimo = n;
    } else {
        b->ultimo->siguiente = n;
        n->anterior = b->ultimo;
        b->ultimo = n;
    }
    b->longitud++;
...
Nodo* blockchain_buscar_por_id(Blockchain *b, unsigned long long id) {
    Nodo *actual = b->primero;
    while (actual) {
...
static int siguiente_potencia_de_dos(int x) {
    int p = 1;
    while (p < x) p <<= 1;
    return p;
}
